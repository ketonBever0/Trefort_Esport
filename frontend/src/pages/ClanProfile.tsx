...
function ClanProfile() {
  ...
  const { getAllUsers, allUsersData } = useContext(UserContext);
  ...
  useEffect(() => {
    ...
    const fetchClanDataByID = async () => {
      await Promise.all([getClanById(id), getClans(), getAllUsers()]);
    };
    if (id) {
      fetchClanDataByID();
    }
          ...
          <div className="row">
            ...
            <div>
              ...
              <div className='row d-flex gap-3 p-5 justify-content-center'>
                ...
                {
                  allUsersData && allUsersData.map((myuser: any, index: any) => (

                    <div>
                      {
                        myuser.userClan[0]?.clan.name == clanDataByID?.name
                          ?
                          <>
